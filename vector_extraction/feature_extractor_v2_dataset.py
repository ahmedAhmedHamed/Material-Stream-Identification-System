...
def process_training_set(
    ...
    results: List[Tuple[np.ndarray, str]] = []
    for _, paths in class_to_train_paths.items():
        results.extend(process_images_parallel(paths, config=config))
        aug_n = int(len(paths) * augmentation_multiplier)
        if aug_n > 0:
            aug_imgs = _generate_augmented_images(paths, aug_n)
...
def build_feature_matrix(
    ...
    X_train, y_train = process_training_set(class_to_train, augmentation_multiplier, config=cfg)
...
def _generate_augmented_images(image_paths: List[str], needed_count: int) -> List[Tuple[np.ndarray, str]]:
    ...
    for _ in range(needed_count):
        ...
        out.append((augment_image(rgb, technique="random"), label))
    return out
